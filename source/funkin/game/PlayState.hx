...
class PlayState extends MusicBeatState
{
	 ...
	 */
	 ...
	 public var ticks:Array<FlxSprite>=[];
	 public var SprBorder:FlxSprite;
	 public var fullBitmap = BitmapData.fromFile('assets/images/game/tacBarFill.png');
	 public var emptyBitmap = BitmapData.fromFile('assets/images/game/tacBarEmpty.png');
	...
	@:dox(hide) override public function create()
	{
		...
		updateRatingStuff();
			...
			for(e in [healthBar, healthBarBG,iconP2, iconP1, scoreTxt, missesTxt, accuracyTxt])
				e.cameras = [camHUD];
	 ...
	 */
	 var currentCorp:Int = 2;

	 public function updateTicks(){
		switch(ticks[currentCorp].animation.curAnim.name){
			case 'Fine':
				ticks[currentCorp].animation.play('Hurt');
				health = 0.01;
			
				trace('corp ${currentCorp} status is hurt');
				case 'Hurt':
					ticks[currentCorp].animation.play('Burst');
					currentCorp--;
					if(currentCorp>-1){
						health = 2;
